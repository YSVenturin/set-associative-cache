...
with open(str(input("Digite o nome do arquivo a ser gerado: ")) + '.txt', 'w') as f:
    ...
    f.write(f"Política de Escrita: {pol_esc}\n"
            ...
            f"Tempo de Leitura/Escrita na Memória Principal: 60\n"
            f"===================================================\n\n")
    ...
    f.write(f"Taxa de Acerto de Leitura: {round(cache.hits_leitura * 100 / arq_ent.num_leituras, 4)}% "
            f"({cache.hits_leitura} certos)\n"
            f"Taxa de Acerto de Escrita: {round(cache.hits_escrita * 100 / arq_ent.num_escritas, 4)}% "
            f"({cache.hits_escrita} acertos)\n"
            f"Taxa de Acerto Global: {round(taxa_de_acerto_global, 4)}% ({cache.hits} acertos)\n"
            f"===================================================\n\n")

    tempo_de_acesso = round((taxa_de_acerto_global/100 * cache.hit_time) + ((1 - taxa_de_acerto_global/100) * (cache.hit_time + 60)), 4)
